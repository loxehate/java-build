 ...
 * Oracle 数据库
 */
public class DbOracle extends AbstractDbTemplate {
    ...
    public void initDbColunmTypes(SqlInfoEntity sqlInfoEntity) {
        ...
        oracleEntity.setJeeLowCodeString("varchar2", 4000);
        oracleEntity.setJeeLowCodeInteger("number", 38);
        oracleEntity.setJeeLowCodeBigInt("number", 38);
        oracleEntity.setJeeLowCodeDate("date", 0);
        oracleEntity.setJeeLowCodeDateTime("timestamp", 0);
        oracleEntity.setJeeLowCodeTime("timestamp", 0);
        oracleEntity.setJeeLowCodeBigDecimal("number", 38,null);
        oracleEntity.setJeeLowCodeText("nclob", 0);
        oracleEntity.setJeeLowCodeLongText("nclob", 0);
    ...
    private String formatIndexAllSql(Map<String, Object> map) {
        ...
        String oracleFormatStr = "select INDEX_NAME AS \"index_name\" from ALL_INDEXES where TABLE_NAME =upper('%s') and INDEX_NAME NOT LIKE 'SYS_%%'";
    ...
    private String formatTableExistSql(String schemaName, Map<String, Object> map) {
        String tableName = FuncBase.getMap2Str(map, "tableName");
        String oracleFormatStr = "SELECT COUNT(*) as \"cou\" FROM ALL_TABLES WHERE OWNER = '%s' AND TABLE_NAME = upper('%s');";
    ...
    private String formatTableFieldCommentSql(String schemaName, Map<String, Object> map) {
        String tableName = FuncBase.getMap2Str(map, "tableName");
        String oracleFormatStr = "SELECT COLUMN_NAME as \"fieldCode\",COMMENTS as \"fieldName\" FROM ALL_COL_COMMENTS WHERE OWNER = '%s' AND TABLE_NAME = upper('%s');";
        return String.format(oracleFormatStr, schemaName, tableName);
    }

    //表名是否存在
    private String formatGetTableNameSql(String schemaName) {
        String oracleFormatStr = "SELECT TABLE_NAME as \"table_name\" FROM ALL_COL_COMMENTS WHERE OWNER = '%s';";
