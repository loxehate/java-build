...
import com.dahuaboke.redisx.from.rdb.hash.HashListPackExParser;
...
public class RdbParserTest {
    ...
    HashListPackExParser hashListPackExParser = new HashListPackExParser();
    ...
    @Test
    public void testHashListPackExParser() {
        Map<byte[], HashExEntry> parse = hashListPackExParser.parse(byteBuf);
        for (Map.Entry<byte[], HashExEntry> kAbdV : parse.entrySet()) {
            String key = new String(kAbdV.getKey(), StandardCharsets.UTF_8);
            HashExEntry valueEntry = kAbdV.getValue();
            String value = new String(valueEntry.getValue(), StandardCharsets.UTF_8);
            System.out.println(key + ":" + value + ":" + valueEntry.getLastTime());
        }
    }
