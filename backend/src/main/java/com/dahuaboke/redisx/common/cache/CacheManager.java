...
public final class CacheManager {
    ...
    private Map<Context, BlockingQueue<SyncCommand>> cache = new ConcurrentHashMap<>();
    ...
    public void registerTo(Context context) {
        ...
        logger.info("Register [To] cache success [{}:{}]", context.getHost(), context.getPort());
    }
    ...
    public void remove(Context context) {
        ...
        if (context instanceof ToContext) {
            ...
            logger.info("Remove [To] cache success [{}:{}]", context.getHost(), context.getPort());
    ...
    public boolean publish(SyncCommand command) {
        ...
        for (Map.Entry<Context, BlockingQueue<SyncCommand>> contextBlockingQueueEntry : cache.entrySet()) {
            ToContext c = (ToContext) contextBlockingQueueEntry.getKey();
            temp.put(c.getHost() + ":" + c.getPort(), c.getSlotBegin() + "-" + c.getSlotEnd());
        }
        logger.error("Key hash not adapt any toContext [{}] \n Now to contexts detail \n {}", command.getStringCommand(), temp);
        return false;
    }
