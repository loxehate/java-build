            ...
            try {
                channel.closeFuture().addListener((ChannelFutureListener) channelFuture -> {
                    if (channelFuture.isSuccess()) {
                        ...
                        logger.info("Close [Console] [{}:{}]", consoleContext.getHost(), consoleContext.getPort());
                    } else {
                        logger.error("Close [Console] error", channelFuture.cause());
                    }
                }).sync();
            } catch (InterruptedException e) {
                logger.error("Close [Console] error", e);
            }
        }
    }
