...
import com.dahuaboke.redisx.from.rdb.hash.HashExEntry;
...
public class CommandParser {
    ...
    private Map<Integer, Type> typeMap = new HashMap() {{
        ...
        put(0x16 & 0xff, Type.HASH);
        put(0x17 & 0xff, Type.HASH);
        put(0x18 & 0xff, Type.HASH);
        put(0x19 & 0xff, Type.HASH);
    }};
        ...
        if (type != null) {
            switch (type) {
                ...
                case ZSET:
                    ...
                    break;
                case HASH:
                    if(rdbData.getRdbType() == 22 || rdbData.getRdbType() == 23
                            || rdbData.getRdbType() == 24 || rdbData.getRdbType() ==25){
                        hashEx(result, rdbData.getKey(), (Map<byte[], HashExEntry>) rdbData.getValue());
                    }else {
    ...
    private void hashEx(List<List<byte[]>> list, byte[] key, Map<byte[], HashExEntry> value) {
        //HSET key field1 "hello" field2 "world"
        List<byte[]> sb = new LinkedList<byte[]>() {{
            add("HSET".getBytes());
            add(key);
        }};
        for (Map.Entry<byte[], HashExEntry> kAbdV : value.entrySet()) {
            byte[] key1 = kAbdV.getKey();
            HashExEntry valueEntry = kAbdV.getValue();
            sb.add(key1);
            sb.add(valueEntry.getValue());
        }
        list.add(sb);

        //HPEXPIRE key milliseconds FIELDS 1 field1, HPEXPIRE key milliseconds FIELDS 1 field2
        for (Map.Entry<byte[], HashExEntry> kAbdV : value.entrySet()) {
            HashExEntry valueEntry = kAbdV.getValue();
            long lastTime = valueEntry.getLastTime();
            //该key没有过期时间/已经过期 跳过
            if(lastTime <= 0L){
                continue;
            }
            List<byte[]> sb2 = new LinkedList<byte[]>() {{
                add("HPEXPIRE".getBytes());
                add(key);
            }};
            byte[] key1 = kAbdV.getKey();
            sb2.add(String.valueOf(lastTime).getBytes());
            sb2.add("FIELDS".getBytes());
            sb2.add("1".getBytes());
            sb2.add(key1);
            list.add(sb2);
        }
