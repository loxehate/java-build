...
public interface ReportMapper extends BaseMapper<ReportEntity> {
    ...
    @JeelowCodeCache(cacheNames = "'FormReportMapperMapper:getById:' + #reportId", reflexClass = ReportEntity.class)
    default ReportEntity getById(Long reportId) {
        LambdaQueryWrapper<ReportEntity> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(ReportEntity::getId, reportId);
        return this.selectOne(wrapper);
    }

    @JeelowCodeCache(cacheNames = "'FormReportMapperMapper:getByCode:' + #reportCode", reflexClass = ReportEntity.class)
    default ReportEntity getByCode(String reportCode) {
        LambdaQueryWrapper<ReportEntity> wrapper = new LambdaQueryWrapper<>();
