 ...
 * Excel导入相关
 */
public interface IExcelFileService extends IService<ExcelFileEntity> {

    //保存Excel数据
    void saveExcelSync( long fileId,ButtonParamImport param);
    ExcelImportResultModel saveExcel(ServletRequestAttributes sra,long fileId, ButtonParamImport param);
    //撤回
    void rollback(Long fileId);
}
